...
import com.example.administrator.smallhappypay.face.widget.FaceRoundView;
...
import com.example.administrator.smallhappypay.face.utils.ImageUtil;
import com.example.administrator.smallhappypay.face.utils.OnResultListener;
import com.example.administrator.smallhappypay.face.widget.WaveHelper;
import com.example.administrator.smallhappypay.face.widget.WaveView;
import com.baidu.idl.facesdk.FaceInfo;
import com.example.administrator.smallhappypay.face.widget.BrightnessTools;
...
import com.example.administrator.smallhappypay.tool.ViewfinderView;
...
import static com.example.administrator.smallhappypay.BaseActivity.LoadingDialog;
import static com.example.administrator.smallhappypay.BaseActivity.dismiss;
...
//    private ViewfinderView rectView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StatusBarUtil.setColor(DetectLoginActivity.this,getResources().getColor(R.color.facecolor),0);
        initScreen();
        initView();
...
//                            startActivity(intent);
                            LoadingDialog(DetectLoginActivity.this,"请稍后");

                            SPUtils.put(DetectLoginActivity.this, "phone", userId);
                            SPUtils.put(DetectLoginActivity.this, "pwd", userInfo);

                            LoginRequest(userId,userInfo);
//                            finish();
                            return;
                        } else {
                            ...
                            if (mDetectCount >= 3) {
                                mDetectTime = false;
                                Toast.makeText(DetectLoginActivity.this, "人脸校验不通过,请确认是否已进行人脸注册", Toast.LENGTH_SHORT).show();
                                finish();
                                return;
                            }
        ...
        Api.gethttpService().getLoginData(acountNo, pwd).enqueue(new Callback<LoginBean>() {
            ...
            public void onResponse(Call<LoginBean> call, Response<LoginBean> response) {
                ...
                if ("00".equals(response.body().getCode())){
                    startActivity(new Intent(DetectLoginActivity.this, MainActivity.class));
                    SPUtils.put(getApplication(),"amNumber",response.body().getMap().getAmNumber());
                    finish();
                }else if ("99".equals(response.body().getCode())){
                    BaseActivity.mdialog(DetectLoginActivity.this,response.body().getFailMessage());
                }
            }
            @Override
            public void onFailure(Call<LoginBean> call, Throwable t) {
                dismiss();
                BaseActivity.mdialog(DetectLoginActivity.this,"服务器维护中");
            }
        });
    }
