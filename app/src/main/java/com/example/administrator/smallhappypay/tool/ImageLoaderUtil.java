...
public abstract class ImageLoaderUtil
{
    ...
    private static class GlideListener<T, K, Z> implements RequestListener<T, K>
    {
        ...
        {
            if (imageLoadListener != null && e!=null)
            {
     ...
     */
    public static void loadImage(final File file, final ImageView imageView, final ImageLoadListener imageLoadListener,float thumbnail, final int... parms)
    {

        DrawableTypeRequest<File> type = Glide.with(imageView.getContext()).load(file);
...
//        }
        if (thumbnail!=0) {
            type.thumbnail(thumbnail);//加载缩略图
            type.dontAnimate();
        }
        type.listener(new GlideListener<File, GlideDrawable, File>(imageLoadListener, file, imageView)).into(imageView);
     ...
     * @param parms             第一个占位符  第二个是error的图片
     */
...
//        }
     ...
     * @param parms             第一个占位符  第二个是error的图片
     */
    public static void loadImageRequest(final String url, final ImageView imageView, final RequestListener requestListener, final int... parms)
    {
        DrawableTypeRequest<String> type = Glide.with(imageView.getContext()).load(url);
...
//        }
     ...
     * @param parms             第一个占位符  第二个是error的图片
     */
    public static void loadImageUri(final Uri uri, final ImageView imageView, final ImageLoadListener imageLoadListener, final int... parms)
    {
        DrawableTypeRequest type = Glide.with(imageView.getContext()).load(uri);
...
//            type.dontAnimate();
//        }
        type.listener(new GlideListener<String, GlideDrawable, Uri>(imageLoadListener, uri, imageView))
                .into(imageView);
    }
