...
public class UnbindOneActivity extends BaseActivity {
    ...
    @BindView(R.id.unbind_imgbtn)
    ImageButton unbindImgbtn;

    private CommonAdapter<MachineInfoBean.MapBean.OneBean> commonAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LoadingDialog(UnbindOneActivity.this,"请稍后");
        getmachineinfo(amNumber);
    }

    @OnClick({R.id.toolbar_back_ll, R.id.unbind_imgbtn})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.toolbar_back_ll:
                finish();
                break;
            case R.id.unbind_imgbtn:
                break;
        }
    }
    private void init(){
        commonAdapter=new CommonAdapter<MachineInfoBean.MapBean.OneBean>(UnbindOneActivity.this,machineInfoBean.getMap().getOne(),R.layout.item_unbing) {
            @Override
            public void convert(final ViewHolder mHolder, final MachineInfoBean.MapBean.OneBean item, final int position) {
        ...
        Api.gethttpService().getMachineInfoData(amNumber).enqueue(new Callback<MachineInfoBean>() {
            ...
            public void onResponse(Call<MachineInfoBean> call, Response<MachineInfoBean> response) {
                ...
                if ("00".equals(response.body().getCode())){
                    if (machineInfoBean.getMap().getOne()!=null){
                        init();
                    }else {
            ...
            public void onFailure(Call<MachineInfoBean> call, Throwable t) {
                dismiss();
                mdialog(UnbindOneActivity.this,"服务器维护中");
            }
        });
    }
