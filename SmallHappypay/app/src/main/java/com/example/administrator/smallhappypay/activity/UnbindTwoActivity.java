...
public class UnbindTwoActivity extends BaseActivity {
    ...
    private CommonAdapter<MachineInfoBean.MapBean.TwoBean> commonAdapter;
    ...
    private void init(){
        commonAdapter=new CommonAdapter<MachineInfoBean.MapBean.TwoBean>(UnbindTwoActivity.this,machineInfoBean.getMap().getTwo(),R.layout.item_unbing) {
            @Override
            public void convert(final ViewHolder mHolder, final MachineInfoBean.MapBean.TwoBean item, final int position) {
        ...
        Api.gethttpService().getMachineInfoData(amNumber).enqueue(new Callback<MachineInfoBean>() {
            ...
            public void onResponse(Call<MachineInfoBean> call, Response<MachineInfoBean> response) {
                ...
                if ("00".equals(response.body().getCode())){
                    if (machineInfoBean.getMap().getTwo()!=null){
                        init();
                    }else {
            ...
            public void onFailure(Call<MachineInfoBean> call, Throwable t) {
                dismiss();
                mdialog(UnbindTwoActivity.this,"服务器维护中");
            }
        });
    }
